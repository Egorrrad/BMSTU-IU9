...
function P_ext(x1, x2, r1, r2)
    return f(x1, x2) + r1 * max(0.0, g1(x1, x2))^2 + r2 * max(0.0, g2(x1, x2))^2
end

function P_vis(x1, x2, r1, r2)
    return f(x1, x2) + r1 * max(0.0, 2 - x1)^2 + r2 * max(0.0, 2 - x2)^2
end
...
# Формирование динамических штрафов 
function build_dynamic_penalties(R; n_phi=9)
    phis = range(pi/4, 0, length=n_phi)
    r1_list = Float64[]
    r2_list = Float64[]
    for p in phis
        push!(r1_list, R * (1 + cos(p)^2))
        push!(r2_list, R * (1 + sin(p)^2))
    end
    return collect(phis), r1_list, r2_list
end
...
function plot_for_R_dynamic(x0, R, filename; x1_range=(0.0, 3.2), x2_range=(0.0, 3.2), n=180, zlim=(0, 120))
    ...
    p = surface(x1s, x2s, zeros(n, n); alpha=0, xlabel="x1", ylabel="x2", zlabel="z", 
                title="R=$R", camera=(28, 20), zlim=zlim, legend=:outertopright, 
                colorbar=false, size=(1100, 800))
    
    F = [f(x1, x2) for x2 in x2s, x1 in x1s]
    surface!(p, x1s, x2s, F; color=:lightgray, alpha=0.22, label="f(x1, x2)")
    
    idxs = [1, Int(cld(length(r1_list), 2)), length(r1_list)]
    surf_colors = [:deepskyblue, :gold, :orangered]
    
    for (k, idx) in enumerate(idxs)
        r1, r2 = r1_list[idx], r2_list[idx]
        Z = [P_vis(x1, x2, r1, r2) for x2 in x2s, x1 in x1s]
        label_str = @sprintf("phi = %.2f", phis[idx])
        surface!(p, x1s, x2s, Z; color=surf_colors[k], alpha=0.34, label=label_str)
    end
    ...
    plot!(p, [x0[1], x1_traj[1]], [x0[2], x2_traj[1]], [z0, z_traj[1]], 
          lw=2.5, ls=:dash, color=:navy, label="")
    
    plot!(p, x1_traj, x2_traj, z_traj; lw=4, color=:magenta, marker=:circle, markersize=5, label="trajectory")
    
    # Точки старта и финиша
    scatter!(p, [x0[1]], [x0[2]], [z0]; ms=16, color=:black, marker=:star5, label="start")
    scatter!(p, [2.0], [2.0], [8.0]; ms=13, color=:green, marker=:diamond, label="(2,2,8)")
    
    savefig(p, filename)
    x_last, y_last = history[end].x1, history[end].x2
    println(@sprintf("R = %.1f -> x* = (%.6f, %.6f), f(x*) = %.6f", R, x_last, y_last, f(x_last, y_last)))
end

# Запуск
x0 = [0.0, 0.0]
R_values = [10.0, 20.0, 50.0, 100.0, 500.0, 1000.0]

for R in R_values
    path = @sprintf("lab14/graphic_R_%0.0f.png", R)
    plot_for_R_dynamic(x0, R, path)
end
