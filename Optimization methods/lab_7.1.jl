...
function build_contour_plot(
    ...
    for i in 1:particle_count
        ...
        plot!(plt, x_track, y_track; color = :black, alpha = 0.18, linewidth = 0.9, label = i == 1 ? "" : "")
    end
...
function run_experiment(function_spec::FunctionSpec, method::MethodSpec)
    ...
    title_text = "$(function_spec.name), $(method.name)"
    ...
    savefig(contour_plot, joinpath("lab15", save_name))

    println("Функция: ", function_spec.name)
    println("Метод: ", method.name)
    println("Итераций: ", result.iterations)
    println("Лучшая позиция: ", result.best_position)
    println("Лучшее значение: ", result.best_value)
    println()
end
...
function run_teleport_probability_experiment(function_spec::FunctionSpec, probabilities::Vector{Float64})
    ...
    savefig(convergence_plot, joinpath("lab15", save_name))
end

function main()
    functions = [
        FunctionSpec("Швейфель", "schwefel", schwefel, [-500.0, -500.0], [500.0, 500.0], 80, 250, 350, 30, 0.03),
        FunctionSpec("Розенброк", "rosenbrock", rosenbrock, [-2.0, -1.0], [2.0, 3.0], 60, 300, 350, 35, 0.03),
        FunctionSpec("Растригин", "rastrigin", rastrigin, [-5.12, -5.12], [5.12, 5.12], 80, 250, 350, 30, 0.03),
    ]

    methods = [
        MethodSpec("Глобальный", :global, 0.35, 0.35, 1.0, 0, ""),
